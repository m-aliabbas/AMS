...
from .models import Profile,ExpiryDays
...
def syncexpiry(request):
    users_to_sync = Profile.objects.filter(user_type=False)
    obj = ExpiryDays.objects.all().last()
    exp_days = obj.days
    for i in users_to_sync:
        new_expiry = i.creation_date + timedelta(days=exp_days)    
        i.expire_date = new_expiry
        i.save()
    context = {
      'sync_msg' : 'Users have been synced successfully!',
    }
    return render(request,'users/sync.html',context)
@login_required
